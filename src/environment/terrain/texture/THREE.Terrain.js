...
(function(global) {
   ...
   Grad.prototype.dot3 = function(x, y, z) {
      return this.x*x + this.z*y + this.z*z;
   };
...
THREE.Terrain = function(options) {
   ...
   var mesh = new THREE.Mesh(
     new THREE.PlaneGeometry(options.xSize, options.zSize, options.xSegments, options.zSegments),
     options.material
   );
...
THREE.Terrain.toArray2D = function(vertices, options) {
   var tgt = new Array(options.xSegments + 1),
       ...
       yl = options.zSegments + 1,
       i, j;
   for (i = 0; i < xl; i++) {
      tgt[i] = new Float32Array(options.zSegments + 1);
      for (j = 0; j < yl; j++) {
...
THREE.Terrain.heightmapArray = function(method, options) {
   var arr = new Array((options.xSegments+1) * (options.zSegments+1)),
       l = arr.length,
       i;
   arr.fill(0);
...
THREE.Terrain.Edges = function(g, options, direction, distance, easing, edges) {
   var numXSegments = Math.floor(distance / (options.xSize / options.xSegments)) || 1,
       numYSegments = Math.floor(distance / (options.zSize / options.zSegments)) || 1,
   ...
   for (i = 0; i < yl; i++) {
      for (j = 0; j < numXSegments; j++) {
         ...
         k1 = i*xl+j;
         k2 = (options.zSegments-i)*xl + (options.xSegments-j);
         if (edges.left) {
            g[k1] = max(g[k1], (peak - g[k1]) * multiplier + g[k1]);
         }
         if (edges.right) {
...
THREE.Terrain.RadialEdges = function(g, options, direction, distance, easing) {
   var peak = direction ? options.maxHeight : options.minHeight,
       ...
       ySegmentSize = options.zSize / options.zSegments,
       edgeRadius = Math.min(options.xSize, options.zSize) * 0.5 - distance,
...
THREE.Terrain.Smooth = function(g, options, weight) {
   ...
   for (var i = 0, xl = options.xSegments + 1, yl = options.zSegments + 1; i < xl; i++) {
      for (var j = 0; j < yl; j++) {
         var sum = 0,
             c = 0;
         for (var n = -1; n <= 1; n++) {
...
THREE.Terrain.Curve = function(g, options, curve) {
   var range = (options.maxHeight - options.minHeight) * 0.5,
       scalar = options.frequency / (Math.min(options.xSegments, options.zSegments) + 1);
   for (var i = 0, xl = options.xSegments + 1, yl = options.zSegments + 1; i < xl; i++) {
      for (var j = 0; j < yl; j++) {
...
THREE.Terrain.Cosine = function(g, options) {
   var amplitude = (options.maxHeight - options.minHeight) * 0.5,
       frequencyScalar = options.frequency * Math.PI / (Math.min(options.xSegments, options.zSegments) + 1),
       phase = Math.random() * Math.PI * 2;
   for (var i = 0, xl = options.xSegments + 1; i < xl; i++) {
      for (var j = 0, yl = options.zSegments + 1; j < yl; j++) {
...
THREE.Terrain.DiamondSquare = function(g, options) {
   ...
   var segments = THREE.Math.ceilPowerOfTwo(Math.max(options.xSegments, options.zSegments) + 1);
...
THREE.Terrain.Fault = function(g, options) {
   var d = Math.sqrt(options.xSegments*options.xSegments + options.zSegments*options.zSegments),
       iterations = d * options.frequency,
       range = (options.maxHeight - options.minHeight) * 0.5,
       displacement = range / iterations,
       smoothDistance = Math.min(options.xSize / options.xSegments, options.zSize / options.zSegments) * options.frequency;
...
THREE.Terrain.Hill = function(g, options, feature, shape) {
   var frequency = options.frequency * 2,
       ...
       smallerSideLength = Math.min(options.xSize, options.zSize),
...
THREE.Terrain.HillIsland = (function() {
   var island = function(coords) {
      ...
      coords.z = 0.5 + Math.sin(theta) * coords.z * 0.4;
   };
...
(function() {
   ...
   THREE.Terrain.Particles = function(g, options) {
      var iterations = Math.sqrt(options.xSegments*options.xSegments + options.zSegments*options.zSegments) * options.frequency * 300,
          xl = options.xSegments + 1,
          displacement = (options.maxHeight - options.minHeight) / iterations * 1000,
          i = Math.floor(Math.random() * options.xSegments),
          j = Math.floor(Math.random() * options.zSegments),
      ...
      for (var k = 0; k < iterations; k++) {
         ...
         if (k % 100 === 0) {
            ...
            j = Math.floor(options.zSegments*(0.5+yDeviation) + Math.sin(d) * Math.random() * options.zSegments*(0.5-Math.abs(yDeviation)));
         }
      }
...
THREE.Terrain.Perlin = function(g, options) {
   ...
   var range = (options.maxHeight - options.minHeight) * 0.5,
       divisor = (Math.min(options.xSegments, options.zSegments) + 1) / options.frequency;
   for (var i = 0, xl = options.xSegments + 1; i < xl; i++) {
      for (var j = 0, yl = options.zSegments + 1; j < yl; j++) {
...
THREE.Terrain.Simplex = function(g, options) {
   noise.seed(Math.random());
   var range = (options.maxHeight - options.minHeight) * 0.5,
       divisor = (Math.min(options.xSegments, options.zSegments) + 1) * 2 / options.frequency;
   for (var i = 0, xl = options.xSegments + 1; i < xl; i++) {
      for (var j = 0, yl = options.zSegments + 1; j < yl; j++) {
...
(function() {
   ...
   function WhiteNoise(g, options, scale, segments, range, data) {
      ...
      for (i = 0, xl = options.xSegments + 1; i < xl; i++) {
         for (j = 0, yl = options.zSegments + 1; j < yl; j++) {
...
THREE.Terrain.generateBlendedMaterial = function(textures, material) {
   ...
   for (var i = 0, l = textures.length; i < l; i++) {
      ...
      if (i !== 0) {
         ...
         assign += '        color = mix( ' +
           'texture2D( texture_' + i + ', MyvUv * vec2( ' + glslifyNumber(tiRepeat.x) + ', ' + glslifyNumber(tiRepeat.z) + ' ) + vec2( ' + glslifyNumber(tiOffset.x) + ', ' + glslifyNumber(tiOffset.z) + ' ) ), ' +
           'color, ' +
   ...
   var fragBlend = 'float slope = acos(max(min(dot(myNormal, vec3(0.0, 0.0, 1.0)), 1.0), -1.0));\n' +
     ...
     '    vec4 color = texture2D( texture_0, MyvUv * vec2( ' + glslifyNumber(t0Repeat.x) + ', ' + glslifyNumber(t0Repeat.z) + ' ) + vec2( ' + glslifyNumber(t0Offset.x) + ', ' + glslifyNumber(t0Offset.z) + ' ) ); // base\n' +
     assign +
     '    diffuseColor = color;\n';
